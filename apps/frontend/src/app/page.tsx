...
export default function HomePage() {
  ...
  const [stickySent, setStickySent] = useState(false);
  ...
  async function sendFreeAssignmentMail({
    email,
    setSending,
    setError,
    onSuccess
    ...
    try {
      ...
      const response = (await apiPost(SEND_FREE_ASSIGNMENT_MAIL_API, {
        to: cleanEmail
      })) as MailApiResponse;

      if (response?.success === false) {
        throw new Error(response.message || "Could not send assessment link.");
      }
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Could not send assessment link. Please try again."
      );
    } finally {
      setSending(false);
    }
  ...
  return (
    <div className="core-root min-h-screen text-slate-600">
      ...
      <main id="top">
        <section className="relative overflow-hidden px-6 pb-20 pt-36 sm:pt-44">
          ...
          <div className="mx-auto max-w-4xl text-center">
            ...
            <h1 className="text-balance text-4xl font-extrabold leading-[1.05] tracking-tight text-slate-900 sm:text-6xl lg:text-7xl" data-testid="home-stop-losing-customers-start-building-ai-agents-heading">
              Stop Losing Customers.<br className="hidden sm:block" />
              <span className="bg-gradient-to-r from-amber-500 via-amber-600 to-amber-700 bg-clip-text text-transparent" data-testid="home-start-building-ai-agents-text">Start Building AI Agents.</span>
            </h1>

            <p className="mx-auto mt-6 max-w-2xl text-pretty text-lg leading-relaxed text-slate-600 sm:text-xl" data-testid="home-businesses-your-missed-calls-and-slow-follow-text">
              <span className="font-semibold text-slate-900" data-testid="home-businesses-text">Businesses:</span> Your missed calls and slow follow-ups are costing you thousands.{" "}
              <span className="font-semibold text-slate-900" data-testid="home-ai-architects-text">AI Architects:</span> build agents that solve these problems and earn 70% of every sale.
            </p>
            ...
            <div className="mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <Link
                data-testid="landing-hero-business-assessment-link"
                href={"/assignment" as Route}
                className="group inline-flex w-full items-center justify-center gap-2 rounded-lg bg-amber-500 px-7 py-3.5 text-base font-semibold text-slate-950 shadow-glow transition hover:scale-[1.03] hover:bg-amber-400 hover:shadow-glow-lg sm:w-auto"
              >
              ...
              <Link
                data-testid="landing-hero-architect-login-link"
                href={"/architect/login" as Route}
                className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-amber-400 px-7 py-3.5 text-base font-semibold text-amber-600 transition hover:scale-[1.03] hover:border-amber-400 hover:bg-amber-50 hover:text-amber-700 sm:w-auto"
              >
            ...
            <div className="mx-auto mt-8 max-w-md rounded-xl border border-amber-200 bg-white p-2 shadow-sm">
              {!heroSent ? (
                <>
                  <form
                    data-testid="landing-hero-email-form"
                    className="flex flex-col gap-2 sm:flex-row"
                    noValidate
                    onSubmit={async (e) => {
                      e.preventDefault();

                      await sendFreeAssignmentMail({
                        email: heroEmail,
                        setSending: setHeroSending,
                        setError: setHeroError,
                        onSuccess: () => setHeroSent(true)
                      });
                    }}
                  >
                    <label data-testid="landing-hero-email-label" htmlFor="hero-email" className="sr-only">
                      Work email
                    </label>

                    <input
                      data-testid="landing-hero-email-input"
                      id="hero-email"
                      type="email"
                      value={heroEmail}
                      ...
                      autoComplete="email"
                      className="w-full rounded-lg border border-amber-200 bg-gray-50 px-4 py-3 text-slate-900 placeholder-slate-500 focus:border-amber-400 focus:outline-none"
                    />

                    <button
                      data-testid="landing-hero-send-assessment-button"
                      type="submit"
                      disabled={heroSending || heroSent}
                      className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-amber-500 px-6 py-3 font-semibold text-slate-950 transition hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-70"
                    >
                      {heroSending ? "Sending..." : heroSent ? "Sent" : "Get Free Assessment"}
                      <ArrowIcon />
                    </button>
                  </form>

                  {heroError ? (
                    <p className="px-2 pt-2 text-center text-xs font-medium text-red-600" data-testid="home-hero-error-text">
                      {heroError}
                    </p>
                  ) : null}
                </>
              ) : (
                <p className="px-2 py-3 text-center text-sm font-semibold text-amber-600" data-testid="home-check-your-inbox-your-assessment-link-is-text">
                  Check your inbox! Your assessment link is on the way.
                </p>
              )}

              <p className="mt-1 px-2 text-center text-xs text-slate-500" data-testid="home-no-credit-card-required-unsubscribe-anytime-text">
          ...
          <div className="relative mx-auto mt-16 max-w-4xl animate-float">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-glow backdrop-blur-sm sm:p-10">
              <div className="mb-8 flex items-center gap-2 border-b border-gray-200 pb-4">
                ...
                <span className="ml-3 font-mono text-xs text-slate-500" data-testid="home-missed-call-recovery-agent-text-4">missed-call-recovery.agent</span>
              </div>
                ...
                <div className="flex flex-col items-center gap-3 rounded-xl border border-amber-400 bg-white px-7 py-6 text-center shadow-sm shadow-glow backdrop-blur md:min-w-[160px]">
                  <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-amber-500 text-slate-950 shadow-glow animate-pulse-ring">
                    <svg className="h-7 w-7" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                      <path d="M12 1.75 14.1 8.4 21 9.3l-5.1 4.1L17.5 21 12 17.2 6.5 21l1.6-7.6L3 9.3l6.9-.9L12 1.75Z" />
                    </svg>
                  </div>
        ...
        <section className="px-6">
          <div className="relative mx-auto mt-16 max-w-4xl animate-float rounded-2xl border border-gray-200 bg-white p-6 shadow-glow backdrop-blur-sm">
            ...
            <p className="mt-4 text-center text-sm text-slate-500" data-testid="home-build-powerful-ai-agents-in-minutes-no-text">
              Build powerful AI agents in minutes. No code required.
            </p>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-14 grid gap-6 lg:grid-cols-2">
              <div id="businesses" className="group scroll-mt-24 rounded-xl border border-gray-200 bg-white p-8 shadow-sm backdrop-blur-sm transition duration-300 hover:-translate-y-1 hover:border-amber-400 hover:shadow-glow sm:p-10">
                <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-xl bg-amber-50 text-amber-600 ring-1 ring-amber-300">
                  <svg className="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                    <path d="M3.75 21h16.5M5.25 21V5.25A1.5 1.5 0 0 1 6.75 3.75h6A1.5 1.5 0 0 1 14.25 5.25V21M14.25 9.75h3.75a1.5 1.5 0 0 1 1.5 1.5V21M8.25 7.5h2.25M8.25 11.25h2.25M8.25 15h2.25" />
                  </svg>
                </div>

                <h3 className="text-xl font-semibold text-slate-900" data-testid="home-for-businesses-heading">For Businesses</h3>
                <p className="mt-3 leading-relaxed text-slate-500" data-testid="home-take-a-2-minute-assessment-to-discover-text">Take a 2-minute assessment to discover which AI agent can recover your lost revenue. We measure and improve:</p>

                <ul className="mt-6 space-y-3">
                  <li className="flex items-center gap-3 text-slate-600" data-testid="home-lead-capture-speed-item"><CheckBadge />Lead Capture Speed</li>
                  <li className="flex items-center gap-3 text-slate-600" data-testid="home-follow-up-consistency-item"><CheckBadge />Follow-Up Consistency</li>
                  <li className="flex items-center gap-3 text-slate-600" data-testid="home-customer-retention-item"><CheckBadge />Customer Retention</li>
                </ul>

                <Link
                  data-testid="landing-business-take-assessment-link"
                  href={"/assignment" as Route}
                  className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-amber-600 transition hover:gap-3 hover:text-amber-600"
                >
              ...
              <div id="architects" className="group scroll-mt-24 rounded-xl border border-gray-200 bg-white p-8 shadow-sm backdrop-blur-sm transition duration-300 hover:-translate-y-1 hover:border-amber-400 hover:shadow-glow sm:p-10">
                <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-xl bg-amber-50 text-amber-600 ring-1 ring-amber-300">
                  <svg className="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                    <path d="M21 7.5 12 2.25 3 7.5m18 0-9 5.25m9-5.25v9L12 21.75M3 7.5l9 5.25M3 7.5v9L12 21.75m0-9v9" />
                  </svg>
                </div>

                <h3 className="text-xl font-semibold text-slate-900" data-testid="home-for-ai-architects-heading">For AI Architects</h3>
                <p className="mt-3 leading-relaxed text-slate-500" data-testid="home-build-ai-agents-visually-with-drag-and-text">Build AI agents visually with drag-and-drop. No coding. Publish to the marketplace. Earn 70% recurring revenue from every business that subscribes.</p>

                <ul className="mt-6 space-y-3">
                  <li className="flex items-center gap-3 text-slate-600" data-testid="home-visual-drag-and-drop-builder-item"><CheckBadge />Visual drag-and-drop builder</li>
                  <li className="flex items-center gap-3 text-slate-600" data-testid="home-publish-to-a-global-marketplace-item"><CheckBadge />Publish to a global marketplace</li>
                  <li className="flex items-center gap-3 text-slate-600" data-testid="home-earn-70-recurring-revenue-item"><CheckBadge />Earn 70% recurring revenue</li>
                </ul>

                <div className="mt-6 rounded-lg border border-amber-200 bg-gray-50 p-5">
                  <div className="text-sm text-slate-500">Agent price: <span className="font-semibold text-slate-600" data-testid="home-99-month-text">$99/month</span></div>
                ...
                <Link
                  data-testid="landing-architect-start-building-link"
                  href={"/architect/login" as Route}
                  className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-amber-600 transition hover:gap-3 hover:text-amber-600"
                >
                  Start building free
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-16">
              <div className="mb-8 flex items-center gap-3">
                <span className="inline-flex items-center rounded-full border border-amber-300 bg-amber-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-amber-600" data-testid="home-for-businesses-text">For Businesses</span>
                ...
                {[
                  ...
                  <div key={n} className="contents">
                    <div className="flex flex-1 items-center gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm backdrop-blur-sm transition hover:border-amber-400 hover:shadow-glow-sm">
                      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-amber-500 font-bold text-slate-950 shadow-glow-sm" data-testid="home-n-text">{n}</span>
                      <div>
                    ...
                    {i < 2 ? (
                      <div className="flex items-center justify-center px-2">
                        <div className="hidden h-px w-8 bg-gradient-to-r from-amber-500/50 to-amber-500/20 md:block" />
                        <div className="h-6 w-px bg-gradient-to-b from-amber-500/50 to-amber-500/20 md:hidden" />
                      </div>
                    ) : null}
                  </div>
                ))}
            ...
            <div className="mt-12">
              <div className="mb-8 flex items-center gap-3">
                <span className="inline-flex items-center rounded-full border border-amber-300 bg-amber-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-amber-600" data-testid="home-for-architects-text">For Architects</span>
                <span className="h-px flex-1 bg-gradient-to-r from-amber-500/20 to-transparent" />
              </div>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:gap-0">
                {[
                  ...
                  <div key={n} className="contents">
                    <div className="flex flex-1 items-center gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm backdrop-blur-sm transition hover:border-amber-400 hover:shadow-glow-sm">
                      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-amber-500 font-bold text-slate-950 shadow-glow-sm" data-testid="home-n-text-2">{n}</span>
                      <div>
                        <div className="font-semibold text-slate-900">{title}</div>
                        <div className="text-sm text-slate-500">{sub}</div>
                      </div>
        ...
        <section id="why" className="scroll-mt-24 px-6 py-20 sm:py-28">
          <div className="mx-auto max-w-7xl">
            <div className="mx-auto max-w-2xl text-center">
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-amber-600" data-testid="home-why-now-text">Why now</p>
              <h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl lg:text-5xl" data-testid="home-built-for-the-ai-agent-era-heading">Built for the AI Agent Era</h2>
            </div>
              ...
              {[
                ...
                <figure key={t.initials} className="flex flex-col rounded-xl border border-gray-200 bg-white p-8 shadow-sm backdrop-blur-sm transition duration-300 hover:-translate-y-1 hover:border-amber-400 hover:shadow-glow">
                  ...
                  <figcaption className="mt-6 flex items-center gap-3 border-t border-gray-200 pt-6">
                    <span className="flex h-10 w-10 items-center justify-center rounded-full bg-amber-100 text-sm font-bold text-amber-600 ring-1 ring-amber-300" data-testid="home-t-initials-text">{t.initials}</span>
                    <span data-testid="home-t-text-2">
                      <span className="block text-sm font-semibold text-slate-900" data-testid="home-t-text-3">{t.name}</span>
                      <span className="block text-xs text-slate-500" data-testid="home-t-role-text">{t.role}</span>
                    </span>
                  </figcaption>
                </figure>
              ))}
        ...
        <section id="assessment" className="scroll-mt-24 px-6 py-20 sm:py-28">
          <div className="relative mx-auto max-w-4xl overflow-hidden rounded-3xl border border-amber-200 bg-white px-6 py-16 text-center shadow-sm backdrop-blur-sm sm:px-12 sm:py-20">
            ...
            <h2 className="text-balance text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl lg:text-5xl" data-testid="home-not-sure-which-ai-agent-your-business-heading">Not Sure Which AI Agent Your Business Needs?</h2>
            <p className="mx-auto mt-5 max-w-2xl text-lg text-slate-600" data-testid="home-take-our-free-2-minute-assessment-answer-text">Take our free 2-minute assessment. Answer 15 questions. Get a personalized recommendation instantly.</p>

            <div className="mt-10">
              <Link
                data-testid="landing-bottom-start-assessment-link"
                href={"/assignment" as Route}
                className="group inline-flex items-center justify-center gap-2 rounded-lg bg-amber-500 px-9 py-4 text-lg font-semibold text-slate-950 shadow-glow-lg transition hover:scale-[1.03] hover:bg-amber-400"
              >
            ...
            <p className="mt-6 text-sm text-slate-500" data-testid="home-takes-2-minutes-completely-free-immediate-results-text">Takes 2 minutes&nbsp;&nbsp;•&nbsp;&nbsp;Completely free&nbsp;&nbsp;•&nbsp;&nbsp;Immediate results</p>

            <div className="mt-4 flex flex-wrap justify-center gap-3">
              <span className="rounded-full border border-amber-200 px-3 py-1 text-xs text-slate-600" data-testid="home-no-credit-card-text">🔒 No credit card</span>
              <span className="rounded-full border border-amber-200 px-3 py-1 text-xs text-slate-600" data-testid="home-instant-results-text">⚡ Instant results</span>
              <span className="rounded-full border border-amber-200 px-3 py-1 text-xs text-slate-600" data-testid="home-personalized-for-your-business-text">🎯 Personalized for your business</span>
            </div>
          </div>
        </section>
      </main>
      ...
      >
        <div className="mx-auto flex w-full max-w-none flex-col items-stretch justify-between gap-2 sm:flex-row sm:items-center sm:gap-4">
          <p className="text-sm text-slate-600" data-testid="home-ready-to-recover-lost-revenue-text">
            Ready to recover lost revenue?
          </p>

          <div className="flex flex-col gap-1">
            <form
              data-testid="landing-sticky-email-form"
              ...
              onSubmit={async (e) => {
                ...
                await sendFreeAssignmentMail({
                  email: stickyEmail,
                  setSending: setStickySending,
                  setError: setStickyError,
                  onSuccess: () => setStickySent(true)
                });
              }}
            >
              <label data-testid="landing-sticky-email-label" htmlFor="sticky-email" className="sr-only">
                Work email
              </label>

              <input
                data-testid="landing-sticky-email-input"
                id="sticky-email"
                type="email"
                value={stickyEmail}
                onChange={(event) => setStickyEmail(event.target.value)}
                placeholder="you@company.com"
                autoComplete="email"
                className="min-w-0 flex-1 rounded-lg border border-amber-200 bg-gray-50 px-4 py-2 text-sm text-slate-900 placeholder-slate-500 focus:border-amber-400 focus:outline-none sm:w-56 sm:flex-none"
              />

              <button
                data-testid="landing-sticky-send-assessment-button"
                type="submit"
                disabled={stickySending || stickySent}
                className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-amber-500 px-5 py-2 text-sm font-semibold text-slate-950 transition hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-70"
              >
                {stickySending ? "Sending..." : stickySent ? "Sent" : "Start Free"}
                <ArrowIcon />
              </button>
            </form>

            {stickyError ? (
              <p className="text-xs font-medium text-red-600" data-testid="home-sticky-error-text">
                {stickyError}
              </p>
            ) : null}

            {stickySent ? (
              <p className="text-xs font-medium text-amber-600" data-testid="home-check-your-inbox-your-assessment-link-is-text-2">
                Check your inbox! Your assessment link is on the way.
              </p>
            ) : null}
          </div>
      ...
      {exitOpen ? (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setExitOpen(false);
          }}
        >
          <div role="dialog" aria-modal="true" aria-labelledby="exit-title" className="relative w-full max-w-md rounded-2xl border border-gray-200 bg-white p-8 shadow-2xl">
            <button
              data-testid="landing-exit-popup-close-button"
              type="button"
              onClick={() => setExitOpen(false)}
              className="absolute right-4 top-4 inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 transition hover:bg-gray-100 hover:text-slate-900"
              aria-label="Close"
            >
            ...
            <h2 id="exit-title" className="pr-8 text-2xl font-bold tracking-tight text-slate-900" data-testid="home-wait-don-apos-t-leave-empty-handed-heading">
              Wait! don&apos;t leave empty-handed.
            </h2>

            <p className="mt-3 text-sm leading-relaxed text-slate-500" data-testid="home-get-your-free-ai-agent-assessment-takes-text">
              Get your free AI agent assessment. Takes 2 minutes. Find out exactly which agents can recover your lost revenue.
            </p>

            {!exitSent ? (
              <>
                <form
                  data-testid="landing-exit-email-form"
                  ...
                  onSubmit={async (e) => {
                    ...
                    await sendFreeAssignmentMail({
                      email: exitEmail,
                      setSending: setExitSending,
                      setError: setExitError,
                      onSuccess: () => setExitSent(true)
                    });
                  }}
                >
                  <label htmlFor="exit-email" className="sr-only" data-testid="home-work-email-label">Work email</label>

                  <input
                    data-testid="landing-exit-email-input"
                    id="exit-email"
                    type="email"
                    required
                    value={exitEmail}
                  ...
                  <button data-testid="landing-submit-button"
                    type="submit"
                    disabled={exitSending}
                    className="inline-flex items-center justify-center gap-2 rounded-lg bg-amber-500 px-6 py-3 font-semibold text-slate-950 transition hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-70"
                  >
                    {exitSending ? "Sending..." : "Send My Free Assessment"}
                  </button>
                </form>

                {exitError ? (
                  <p className="mt-2 text-center text-xs font-medium text-red-600" data-testid="home-exit-error-text">
                    {exitError}
                  </p>
                ) : null}
              </>
            ) : (
              <p className="mt-4 text-center text-sm font-semibold text-amber-600" data-testid="home-check-your-inbox-your-assessment-link-is-text-3">
                Check your inbox! Your assessment link is on the way.
              </p>
            )}

            <p className="mt-4 text-center text-xs text-slate-500" data-testid="home-no-spam-unsubscribe-anytime-text">
