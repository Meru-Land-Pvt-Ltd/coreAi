    ...
    <div className="core-root min-h-screen text-slate-600">
      ...
      >
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <Link href="/" className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 transition hover:text-slate-900">
            <IconBack />
            Back to Home
          </Link>
        </nav>
        ...
        {screen === "intro" && (
          <section className="screen animate-fade-up px-6 pb-24 pt-32">
            <div className="mx-auto max-w-2xl text-center">
              ...
              <h1 className="text-balance text-4xl font-extrabold leading-[1.08] tracking-tight text-slate-900 sm:text-5xl">Your AI Agent Readiness Assessment</h1>
              <p className="mx-auto mt-4 max-w-xl text-lg leading-relaxed text-slate-600">Answer 15 quick questions to discover which AI agent can recover your lost revenue. Takes less than 2 minutes.</p>

              <div className="mt-6 flex flex-wrap justify-center gap-3">
                <span className="rounded-full border border-amber-200 px-3 py-1 text-xs text-slate-600">📊 Get your score</span>
                <span className="rounded-full border border-amber-200 px-3 py-1 text-xs text-slate-600">🎯 Personalized recommendation</span>
                <span className="rounded-full border border-amber-200 px-3 py-1 text-xs text-slate-600">⚡ Instant results</span>
              </div>
            </div>

            <form onSubmit={handleIntroSubmit} className="mx-auto mt-10 max-w-xl space-y-4 rounded-xl border border-gray-200 bg-white p-6 text-left shadow-sm backdrop-blur-sm sm:p-8" noValidate>
              <div>
                <label htmlFor="f-name" className="mb-1.5 block text-sm font-medium text-slate-600">Full name</label>
                <input
                  id="f-name"
                  name="name"
                  type="text"
                ...
                />
                {errName && <p className="mt-1.5 text-xs text-red-600">Please enter your name.</p>}
              </div>
              <div>
                <label htmlFor="f-email" className="mb-1.5 block text-sm font-medium text-slate-600">Work email</label>
                <input
                  id="f-email"
                  name="email"
                  type="email"
                ...
                />
                {errEmail && <p className="mt-1.5 text-xs text-red-600">Please enter a valid email address.</p>}
              </div>
              <div>
                <label htmlFor="f-phone" className="mb-1.5 block text-sm font-medium text-slate-600">
                  Phone number <span className="font-normal text-slate-500">— Optional, for SMS results</span>
                </label>
                <input
                  id="f-phone"
                  name="phone"
              ...
              <div>
                <label htmlFor="f-biz" className="mb-1.5 block text-sm font-medium text-slate-600">Business type</label>
                <select
                  id="f-biz"
                  name="business"
              ...
              <div className="pt-2">
                <button type="submit" className="inline-flex w-full max-w-sm items-center justify-center gap-2 rounded-lg bg-amber-500 px-6 py-3.5 text-base font-semibold text-slate-950 shadow-glow transition hover:scale-[1.02] hover:bg-amber-400">
                  Start My Assessment
                  <IconArrow />
                </button>
                <p className="mt-3 text-xs text-slate-500">🔒 Your information is private and never shared.</p>
              </div>
            </form>
          </section>
        )}
        ...
        {screen === "questions" && (
          <section className="screen animate-fade-up px-6 pb-24 pt-32">
            <div className="mx-auto max-w-2xl">
              ...
              <p className="mt-2 text-xs text-slate-500">Question {current + 1} of {QUESTIONS.length}</p>
              ...
              <div key={q.id} className="mt-6 animate-fade-up">
                <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm backdrop-blur-sm sm:p-8">
                  <button type="button" onClick={() => advance(-1, current)} className="mb-5 inline-flex items-center gap-1 text-sm text-slate-500 transition hover:text-slate-900">
                    <IconBack />
                    {current === 0 ? "Back to start" : "Back"}
                  </button>
                  <h2 className="text-xl font-semibold leading-snug text-slate-900 sm:text-2xl">{q.text}</h2>
                      ...
                      {q.type === "yesno"
                        ...
                        : (q.options || []).map((o) => {
                            ...
                            return (
                              ...
                              >
                                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-amber-300 bg-amber-50 text-xs font-bold text-amber-600">{o.key}</span>
          ...
          <section className="screen animate-fade-up px-6 pb-24 pt-32">
            <div className="mx-auto max-w-4xl">
              <div className="text-center">
                <p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-amber-600">Your assessment results</p>
                <h1 className="text-balance text-3xl font-extrabold tracking-tight text-slate-900 sm:text-4xl">{firstName()}, here&apos;s where your business stands</h1>
              </div>
                ...
                <div className="relative h-52 w-52">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <div className="flex items-end gap-1">
                      <span className={`text-6xl font-extrabold ${results.tier.cls}`}>{displayScore}</span>
                      <span className="mb-2 text-xl font-semibold text-slate-500">/ 10</span>
                    </div>
                    <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Readiness</span>
                  </div>
                </div>
                <p className={`mt-6 max-w-md text-center text-lg font-semibold ${results.tier.cls}`}>{results.tier.label}</p>
              </div>

              <div className="mt-12">
                <h2 className="text-center text-sm font-semibold uppercase tracking-[0.18em] text-slate-500">What we found</h2>
                <div className="mt-6 grid gap-4 sm:grid-cols-3">
                  {results.insights.map((i, idx) => (
                    <div key={idx} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm backdrop-blur-sm">
                      ...
                      <p className="mt-1 text-sm leading-relaxed text-slate-500">{i.desc}</p>
                    </div>
                  ))}
                </div>
              </div>

              <div className="mt-10 rounded-xl border border-amber-400 bg-amber-50 p-8 shadow-glow">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-600">Recommended for you</p>
                <h3 className="mt-2 text-2xl font-bold text-slate-900">{results.rec[0]}</h3>
                <p className="mt-2 leading-relaxed text-slate-600">{results.rec[1]}</p>
              </div>

              <div className="mt-10 text-center">
                <a href="#" className="inline-flex items-center justify-center gap-2 rounded-lg bg-amber-500 px-8 py-4 text-lg font-semibold text-slate-950 shadow-glow-lg transition hover:scale-[1.03] hover:bg-amber-400">
                  {results.cta[0]}
                  <IconArrow />
                </a>
                <p className="mx-auto mt-4 max-w-md leading-relaxed text-slate-500">{results.cta[1]}</p>
                <p className="mt-6 text-sm text-slate-500">
                  Your full report has been sent to <span className="font-medium text-slate-600">{contact.email || "your inbox"}</span>. Check your inbox.
                </p>
                <button type="button" onClick={retake} className="mt-8 text-xs text-slate-500 underline-offset-4 transition hover:text-slate-600 hover:underline">Retake the assessment</button>
              </div>
            </div>
          </section>
        )}
          ...
          <div className="grid grid-cols-2 gap-10 md:grid-cols-5">
            ...
            <div>
              ...
              <ul className="mt-4 space-y-3 text-sm">
                <li><Link href="/#platform" className="text-slate-500 transition hover:text-amber-600">Platform</Link></li>
                <li><Link href="/#assessment" className="text-slate-500 transition hover:text-amber-600">Pricing</Link></li>
