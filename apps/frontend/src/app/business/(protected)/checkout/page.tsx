  ...
  return (
    <div className="checkout-root min-h-screen bg-gray-50 text-slate-900">
      ...
      {!confirmation ? (
        ...
        <ConfirmationView
          email={email}
          onSetup={() => router.push("/business/agent/missed-call/setup")}
          onDashboard={() => router.push("/business/marketplace")}
        />
      )}
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-gray-100 bg-white px-5 py-4 sm:px-8">
      <div className="mx-auto grid max-w-6xl grid-cols-3 items-center gap-2">
        <Link href="/business/marketplace" className="group flex w-fit items-center gap-2.5">
          <span
          ...
          <nav className="flex items-center justify-center" aria-label="Checkout progress">
            ...
            <span
              className={`ml-2 hidden text-sm sm:inline ${
                confirmation ? "font-semibold text-amber-600" : "font-bold text-amber-600"
              }`}
            >
              Payment
            </span>
            ...
            <span
              className={`ml-2 hidden text-sm font-semibold sm:inline ${
                confirmation ? "text-amber-600" : "text-slate-400"
              }`}
            >
              Confirmation
            </span>
    ...
    <div className="flex justify-between text-sm">
      ...
      <span
        className={`tnum font-medium ${
          green ? "text-green-600" : muted ? "text-slate-400" : "text-slate-900"
        }`}
      >
        {value}
      </span>
