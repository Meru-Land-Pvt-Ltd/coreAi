...
/**
 * WHERE OUR MAIL COMES FROM.
 *
 * The per-run ceiling used to live here too, drawn a second time beside the
 * generic dials panel that already draws it. See EmailLimitsPanel below.
 */
...
/**
 * TWO CONTROLS, ONE SETTING, DIFFERENT ANSWERS.
 *
 * "Most emails in one run" was drawn twice on the same screen — once here
 * with the options 5/10/25/50/100/200, and once by the generic dials panel
 * above it, which reads the node's own declaration and offers different
 * numbers. Both saved to `emailMaxPerRun`. Whichever an admin touched last
 * won, and the other went on showing the old figure until the page reloaded,
 * so the screen disagreed with itself about the platform's own ceiling.
 *
 * The node's row is the single truth, and the generic panel draws it. What is
 * left here is the one thing that panel cannot draw: which domains our mail
 * may be sent from.
 */
