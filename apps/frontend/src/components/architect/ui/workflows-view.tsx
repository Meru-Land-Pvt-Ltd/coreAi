...
function CreateAgentCard({ onClick, compact = false }: { onClick: () => void; compact?: boolean }) {
  return (
    <button data-testid="workflows-create-new-agent"
      type="button"
      onClick={onClick}
      className={cn(
    ...
    <div className="-m-4 min-h-screen w-auto max-w-none bg-white p-4 sm:-m-6 sm:p-6 lg:-m-8 lg:p-8">
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-3">
        <div className="border-l-4 border-amber-500 bg-amber-50/40 px-5 py-4">
          <p className="text-sm font-semibold text-slate-500" data-testid="architect-ui-workflows-view-agents-text">Agents</p>
          <p className="mt-2 text-3xl font-black text-slate-950" data-testid="architect-ui-workflows-view-workflows-text">{workflows.length}</p>
        </div>
      </div>
      ...
      {loading ? (
        <section className="border-l-4 border-amber-500 bg-amber-50/40 px-5 py-4">
          <p className="text-sm font-bold text-amber-700" data-testid="architect-ui-workflows-view-loading-agents-text">Loading agents...</p>
        </section>
      ) : workflows.length ? (
              ...
              >
                <div className="absolute right-5 top-5 flex items-center gap-2">
                  <span
                   ...
                   data-testid="architect-ui-workflows-view-status-label-text">
                    {status.label}
                  </span>

                  <button data-testid="workflows-open-template"
                    type="button"
                    onClick={(event) => {
                ...
                <div className="mt-8 flex flex-1 flex-col items-center justify-center">
                  <h2 className="max-w-[18rem] text-center text-xl font-black leading-tight text-slate-950" data-testid="architect-ui-workflows-view-get-workflow-display-workflow-heading">
                    {getWorkflowDisplayName(workflow)}
                  </h2>

                  <p className="mt-3 line-clamp-3 max-w-sm text-center text-sm font-semibold leading-6 text-slate-500" data-testid="architect-ui-workflows-view-workflow-description-enoylity-media-creations-llc-text">
                    {workflow.description || "Enoylity Media Creations LLC"}
                  </p>
                </div>

                <p className="mt-6 text-xs font-semibold text-slate-400" data-testid="architect-ui-workflows-view-created-format-date-workflow-created-at-text">
      ...
      {isCreateOpen ? (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/45 p-4 backdrop-blur-sm sm:p-8" role="dialog" aria-modal="true" onClick={() => setIsCreateOpen(false)}>
          <div className="relative my-auto max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-[2rem] border border-slate-200 bg-white p-4 shadow-2xl shadow-slate-950/20 sm:p-6" onClick={(event) => event.stopPropagation()}>
            <button data-testid="workflows-close-template-modal" type="button" onClick={() => setIsCreateOpen(false)} aria-label="Close" className="absolute right-4 top-4 z-10 grid h-9 w-9 place-items-center rounded-full border border-slate-200 bg-white text-lg font-black text-slate-700 transition hover:bg-slate-50">
              ×
            </button>
