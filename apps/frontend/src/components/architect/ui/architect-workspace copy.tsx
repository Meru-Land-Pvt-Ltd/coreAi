...
export function ArchitectWorkspace() {
  ...
  async function handleListingSubmit(formData: FormData) {
    ...
    if (!workflowId) {
      setMessage("Choose a workflow first. Marketplace publishing now happens from Workflow Configure.");
      return;
    }
    router.push(`/architect/workflows/${encodeURIComponent(workflowId)}/builder?tab=configure` as Route);
  }
            ...
            <section className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
              <Panel title="Submit Agent Listing">
                <form data-testid="architect-workspace-listing-form" action={handleListingSubmit} className="grid gap-4">
                  ...
                  <p className="text-sm text-slate-600">Industry, Subindustry, buyer setup, integrations, pricing, and listing copy are configured in the workflow Configure tab.</p>
                  <SubmitButton>Continue to Configure</SubmitButton>
                </form>
              </Panel>
