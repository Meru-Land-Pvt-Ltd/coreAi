...
function SelectBox({ value, onChange, options }: { value: string; onChange: (value: string) => void; options: string[] }) {
  // Keep the current value selectable even if it isn't one of the listed options.
    ...
    <>
      ...
      <Section title="Event">
        ...
        <div className="mt-4">
          ...
          <TextArea mono height="h-20" value={str("eventDescription")} onChange={set("eventDescription")} placeholder="Phone: [Customer Phone]\nService: [Service]" />
        </div>
      </Section>
