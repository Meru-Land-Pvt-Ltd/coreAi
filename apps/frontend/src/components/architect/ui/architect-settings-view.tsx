            ...
            {activeTab === "profile" ? (
              <div className="space-y-5">
                <div className="flex items-center gap-5">
                  <button type="button" className="relative grid h-20 w-20 place-items-center rounded-full bg-gradient-to-br from-amber-400 to-orange-500 text-xl font-black text-white shadow-lg shadow-amber-500/25">
                    MT
                  </button>
                  <div>
                    <p className="font-black text-slate-950">Profile photo</p>
                    <p className="mt-1 text-sm text-slate-500">Used across your architect account.</p>
                    <button type="button" className="mt-2 text-sm font-black text-amber-700">Change photo</button>
                  </div>
                </div>
            ...
            {activeTab === "security" ? (
              <div className="space-y-4">
                ...
                <SettingsRow title="Active session" description="Chrome on macOS · San Francisco · Current session">
                  <button type="button" onClick={() => save("Other sessions revoked")} className="rounded-xl border border-red-100 bg-red-50 px-4 py-2 text-sm font-black text-red-600">Revoke others</button>
                </SettingsRow>
                <SettingsRow title="Password" description="Last changed 38 days ago.">
                  <button type="button" className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-black text-slate-700">Change password</button>
                </SettingsRow>
              </div>
            ) : null}
            ...
            {activeTab === "developer" ? (
              <div className="space-y-4">
                ...
                <div className="rounded-2xl bg-slate-950 p-4 font-mono text-xs text-slate-300">
                  <p>CORE_ARCHITECT_KEY=sk_test_••••••••••••</p>
                  <p className="mt-2 text-slate-500">Webhook: https://api.core.local/architect/hooks</p>
                </div>
            ...
            {activeTab === "data" ? (
              <div className="space-y-4">
                <SettingsRow title="Export account data" description="Download storefront, listings, workflow metadata, and payout records.">
                  <button type="button" onClick={() => save("Data export requested")} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-black text-slate-700">Export</button>
                </SettingsRow>
                <SettingsRow title="Delete architect account" description="Permanently delete profile, listings, and account data after review.">
                  <button type="button" className="rounded-xl border border-red-100 bg-red-50 px-4 py-2 text-sm font-black text-red-600">Request deletion</button>
                </SettingsRow>
              </div>
            ) : null}
          </ArchitectCard>
