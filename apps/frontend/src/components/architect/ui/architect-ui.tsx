    ...
    <div className="mb-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="relative px-5 py-5 sm:px-6 lg:px-7">
        ...
        <div className="relative flex flex-col justify-between gap-5 md:flex-row md:items-center">
          <div>
            <p className="text-[11px] font-extrabold uppercase tracking-[0.22em] text-amber-600" data-testid="architect-ui-architect-ui-eyebrow-text">
              {eyebrow}
            </p>
            <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950 md:text-3xl" data-testid="architect-ui-architect-ui-title-heading">
              {title}
            </h1>
            {description ? (
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500" data-testid="architect-ui-architect-ui-description-text">
                {description}
              </p>
            ) : null}
          </div>
          ...
          {(actionHref && actionLabel) || (secondaryActionHref && secondaryActionLabel) ? (
            <div className="flex flex-wrap gap-2.5">
              {secondaryActionHref && secondaryActionLabel ? (
                <Link data-testid="architect-landing-business-link"
              ...
              {actionHref && actionLabel ? (
                <Link data-testid="architect-landing-login-link"
    ...
    <section className={cn("rounded-3xl border border-slate-200 bg-white p-5 shadow-sm", className)}>
      {title || description || action ? (
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            {title ? <h2 className="text-lg font-extrabold text-slate-950" data-testid="architect-ui-architect-ui-title-heading-2">{title}</h2> : null}
            {description ? (
              <p className="mt-1 text-sm leading-6 text-slate-500" data-testid="architect-ui-architect-ui-description-text-2">{description}</p>
            ) : null}
          </div>
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-slate-500" data-testid="architect-ui-architect-ui-label-text">{label}</p>
          <h3 className="mt-2 text-3xl font-black tracking-tight text-slate-950" data-testid="architect-ui-architect-ui-2-3xl-tracking-tight-heading">{value}</h3>
        </div>
        {icon ? (
          <span className={cn("grid h-11 w-11 place-items-center rounded-2xl ring-1", toneMap[tone])} data-testid="architect-ui-architect-ui-icon-text">
            {icon}
          </span>
        ) : null}
      </div>
      {hint ? <p className="mt-3 text-xs font-semibold leading-5 text-slate-500" data-testid="architect-ui-architect-ui-hint-text">{hint}</p> : null}
    </div>
  );
}
...
export function ArchitectEmptyState({ title, text, actionLabel, actionHref }: EmptyStateProps) {
  return (
    <div className="rounded-3xl border border-dashed border-amber-200 bg-amber-50/60 p-8 text-center">
      ...
      <h3 className="mt-4 text-lg font-black text-slate-950" data-testid="architect-ui-architect-ui-title-heading-3">{title}</h3>
      <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-500" data-testid="architect-ui-architect-ui-auto-2-max-lg-text">{text}</p>
      {actionHref && actionLabel ? (
        <Link data-testid="architect-landing-learn-more-link"
          href={actionHref as Route}
...
export function ArchitectBadge({ children, tone = "amber" }: { children: ReactNode; tone?: keyof typeof toneMap }) {
  return (
    <span data-testid={`architect-badge-${tone}`} className={cn("inline-flex rounded-full px-3 py-1 text-xs font-extrabold uppercase tracking-wide ring-1", toneMap[tone])}>
      {children}
    </span>
  );
}
...
export function ArchitectField({
  ...
  return (
    <label className="grid gap-2 text-sm font-bold text-slate-800" data-testid="architect-ui-architect-ui-label-label">
      {label}
      <input data-testid="architect-landing-email-input"
        name={name}
        type={type}
...
export function ArchitectTextarea({
  ...
  return (
    <label className="grid gap-2 text-sm font-bold text-slate-800" data-testid="architect-ui-architect-ui-label-label-2">
      {label}
      <textarea data-testid="architect-landing-message-textarea"
        name={name}
        placeholder={placeholder}
        defaultValue={defaultValue}
        required={required}
...
export function ArchitectPrimaryButton({ children, disabled, type = "submit", className, ...props }: PrimaryButtonProps) {
  return (
    <button data-testid="architect-landing-submit"
      disabled={disabled}
      type={type}
      {...props}
      className={cn(
...
export function ArchitectSecondaryButton({ children, disabled, type = "button", className, ...props }: PrimaryButtonProps) {
  return (
    <button data-testid="architect-landing-secondary-submit"
      disabled={disabled}
      type={type}
      {...props}
      className={cn(
...
export function ArchitectLinkButton({ href, children }: { href: Route; children: ReactNode }) {
  return (
    <Link data-testid="architect-landing-bottom-cta-link"
      href={href}
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between text-xs font-bold text-slate-500">
        <span data-testid="architect-ui-architect-ui-label-progress-text">{label ?? "Progress"}</span>
        <span data-testid="architect-ui-architect-ui-safe-text">{safeValue}%</span>
      </div>
