          ...
          <ArchitectCard title="Listing Preview">
            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-950">Marketplace card</h3>
                  <p className="mt-1 text-sm text-slate-500">{selectedWorkflow?.name ?? "Select a workflow to anchor this agent."}</p>
                </div>
              </div>
            ...
            <div className="space-y-3">
              {publishSteps.map((step, index) => (
                <div key={step} className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-100">
                  <span className="grid h-8 w-8 place-items-center rounded-full bg-amber-500 text-xs font-black text-slate-950">{index + 1}</span>
