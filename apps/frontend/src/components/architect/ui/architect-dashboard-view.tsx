...
export function ArchitectDashboardView() {
  ...
  if (loading) {
    return (
      <ArchitectCard>
        <p className="text-sm font-bold text-amber-700">Loading architect dashboard...</p>
      </ArchitectCard>
    );
  }
        ...
        <ArchitectCard title="Profile strength" description="Complete your storefront profile to improve buyer trust.">
          <div className="rounded-2xl bg-gradient-to-br from-amber-50 to-white p-4 ring-1 ring-amber-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-black text-slate-950">{summary?.user.fullName ?? "Architect"}</p>
                <p className="text-sm font-semibold text-slate-500">{summary?.profile?.title ?? "AI Workflow Architect"}</p>
              </div>
            </div>
            <div className="mt-5">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-white p-3 ring-1 ring-slate-100">
                ...
                <p className="mt-1 text-xl font-black text-slate-950">{summary?.profile?.rating ?? 0}/5</p>
              </div>
              <div className="rounded-xl bg-white p-3 ring-1 ring-slate-100">
                <p className="text-xs font-bold text-slate-500">Jobs</p>
                <p className="mt-1 text-xl font-black text-slate-950">{summary?.profile?.completedJobs ?? 0}</p>
              </div>
            </div>
            <Link href="/architect/profile" className="mt-5 inline-flex w-full justify-center rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-black text-white transition hover:bg-slate-800">
              Update profile
            </Link>
          </div>
        </ArchitectCard>
          ...
          {recentWorkflows.length ? (
            <div className="space-y-3">
              {recentWorkflows.slice(0, 4).map((workflow) => (
                <Link key={workflow.id} href={`/architect/workflows/${workflow.id}/builder` as Route} className="block rounded-2xl border border-slate-100 p-4 transition hover:border-amber-200 hover:bg-amber-50/40">
                  <p className="font-black text-slate-950">{workflow.name}</p>
                  <p className="mt-1 text-xs font-semibold text-slate-500">Created {formatDate(workflow.createdAt)}</p>
                </Link>
              ))}
            </div>
          ) : (
              ...
              {recentListings.slice(0, 4).map((listing) => (
                <div key={listing.id} className="rounded-2xl border border-slate-100 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-black text-slate-950">{listing.name}</p>
                      <p className="mt-1 line-clamp-2 text-sm text-slate-500">{listing.shortDescription}</p>
                    </div>
                    <ArchitectStatusPill status={listing.status} />
                  </div>
                  <p className="mt-3 text-sm font-black text-slate-950">{formatMoney(listing.priceCents)}</p>
                </div>
              ))}
            </div>
          ) : (
        ...
        <ArchitectCard title="Proposal activity" className="xl:col-span-1">
          {recentProposals.length ? (
              ...
              {recentProposals.slice(0, 4).map((proposal) => (
                <div key={proposal.id} className="rounded-2xl border border-slate-100 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <p className="font-black text-slate-950">{proposal.project.title}</p>
                    <ArchitectStatusPill status={proposal.status} />
                  </div>
                  <p className="mt-2 text-sm text-slate-500">{proposal.etaDays ? `${proposal.etaDays} day ETA` : "ETA not set"}</p>
                </div>
              ))}
            </div>
          ) : (
            <div className="rounded-2xl bg-slate-50 p-5">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-500">Project proposal history will appear here when businesses start requesting custom builds.</p>
            </div>
          )}
        </ArchitectCard>
      </div>
