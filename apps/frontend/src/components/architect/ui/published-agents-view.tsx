...
export function PublishedAgentsView() {
  ...
  return (
    <div>
      ...
      <ArchitectCard>
        {loading ? (
          <p className="text-sm font-bold text-amber-700">Loading published agents...</p>
        ) : agents.length ? (
              ...
              <article key={agent.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-amber-200 hover:shadow-lg hover:shadow-slate-900/5">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm font-bold text-amber-700">CORE Marketplace Agent</p>
                  </div>
                </div>

                <p className="mt-4 line-clamp-3 text-sm leading-6 text-slate-500">{agent.shortDescription}</p>

                <div className="mt-4 flex flex-wrap gap-2">
                  {agent.tags.map((tag) => (
                    <span key={`${agent.id}-${tag}`} className="rounded-full bg-amber-50 px-3 py-1 text-xs font-black text-amber-700 ring-1 ring-amber-100">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-5 flex items-end justify-between border-t border-slate-100 pt-4">
                  <div>
                    <p className="text-2xl font-black text-slate-950">{formatMoney(agent.priceCents)}</p>
                    <p className="mt-1 text-xs font-semibold text-slate-500">Published from {formatDate(agent.createdAt)}</p>
                  </div>
                  <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black uppercase tracking-wide text-emerald-700 ring-1 ring-emerald-100">Live</span>
                </div>
              </article>
            ))}
          </div>
