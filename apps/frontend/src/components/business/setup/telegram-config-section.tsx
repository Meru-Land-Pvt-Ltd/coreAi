...
import { HelpCircle, Info, Plus, Trash2, Zap, Sparkles, UploadCloud, FileText, Loader2 } from "lucide-react";
import {
...
const CUSTOM_COMMAND_ACTIONS: Array<{
  ...
  },
  {
    value: "services",
    label: "Booking Flow",
    subtitle: "Show list of bookable services and start appointment scheduling"
  }
];
...
function ensureDefaultCommands(loadedCmds: TelegramBusinessCustomCommand[]): TelegramBusinessCustomCommand[] {
  const normalized = loadedCmds.map((cmd) =>
    cmd.command === "commands" && !cmd.response.trim()
      ? { ...cmd, response: "Show a list of all active bot commands." }
      : cmd
  );
  const hasCommands = normalized.some((c) => c.command === "commands");
  if (!hasCommands) {
    return [
      {
        command: "commands",
...
export function TelegramConfigSection({
  ...
  const handleGenerateCommands = async () => {
    ...
    try {
      const res = await generateBusinessTelegramCommands(
        installedAgentId,
        businessInfoInput || undefined,
        selectedFile || undefined
      );

      if (!res.success || !res.data) {
        throw new Error(res.error || "Failed to generate Telegram commands.");
      }
      ...
      setSettings(prev => {
        ...
        return {
          ...prev,
          telegramWelcomeMessage: data.welcomeMessage || prev.telegramWelcomeMessage,
          telegramFallbackMessage: data.fallbackMessage || prev.telegramFallbackMessage,
