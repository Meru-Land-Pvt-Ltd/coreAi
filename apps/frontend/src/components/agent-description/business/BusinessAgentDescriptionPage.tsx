...
function getTrialInfo(purchasedAt: string, status: string, isTrialProp?: boolean, trialDaysLimit?: number | null) {
  ...
  const trialEnded =
    isTrial && (status.toUpperCase() === "FAILED" || status.toUpperCase() === "CANCELED" || daysLeft <= 0);
...
export default function BusinessAgentDescriptionPage() {
  ...
  useEffect(() => {
    ...
    async function loadOwnedAgent() {
      try {
        ...
        const entry = (response.data?.agents ?? []).find((agent) => agent.listing.id === agentId);
  ...
  const installsLabel = formatRealInstallCount(
    listing.installCount ?? listing.architect?.architectProfile?.completedJobs ?? 0
  );
  ...
  const trialDays = listing.trialDays ?? TRIAL_DAYS;
  ...
  const showPayButton = Boolean(
    ownedAgent && (trialInfo?.isTrial || trialInfo?.trialEnded || purchaseStatus === "TRIALING" || isPaymentFailed)
  );
  ...
  const isSetupCompleted =
    Boolean(ownedAgent?.installedAgentId) && (installedAgentStatus === "ACTIVE" || installedAgentStatus === "PAUSED");

  const canStartTrial = !ownedAgent && pricingModel !== "FREE" && freeTrialEnabled && trialDays > 0;
  const hasActiveAccess = Boolean(ownedAgent);

  let primaryCtaHref: string = checkoutPath;
  let primaryCtaLabel: string;
  let primaryCtaTestId: string;

  if (showSetupButton) {
    primaryCtaHref = setupPath;
    primaryCtaLabel = isSetupCompleted ? "Edit Configuration" : "Set up agent";
    primaryCtaTestId = "owned-agent-detail-setup";
  } else if (showPayButton) {
    primaryCtaHref = checkoutPath;
    primaryCtaLabel = pricingModel === "ONE_TIME" ? `Buy now — $${price}` : `Pay $${price}/mo`;
    primaryCtaTestId = "owned-agent-detail-pay-now";
  } else {
    primaryCtaHref = checkoutPath;
    primaryCtaLabel =
      pricingModel === "FREE"
        ...
        : pricingModel === "ONE_TIME"
          ...
          : canStartTrial
            ...
            : `Subscribe — $${price}/mo`;
    primaryCtaTestId = "agent-detail-get-agent";
  }
  ...
  return (
    <AgentDescriptionView
      ...
      category={category}
      statusLabel={ownedAgent ? "In your account" : "Ready to install"}
      showAuthor
      author={author}
      ...
      similarHref={(id) => businessAgentDetailPath(id)}
      showDemo
