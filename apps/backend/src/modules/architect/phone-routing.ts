...
export function forwardingInstructions(
  ...
  const target = aiNumber || "your assigned Triven AI number";
...
/** Assign or reuse a unique Triven AI number for this business (no duplicate mappings). */
async function ensureForwardingNumber(
  ...
  throw new Error("No Triven AI phone number available. Seed PlatformPhoneNumber rows or set TWILIO_PHONE_NUMBER.");
}
...
export async function testPhoneRouting(input: { called?: unknown; from?: unknown }) {
  ...
  if (!phone?.installedAgent) {
    return {
      ...
      matchedNumber: null,
      callerNumber,
      message: `No business is mapped to ${called}. Assign/deploy a Triven AI number first.`
    };
  }
