...
const envSchema = z.object({
  ...
  // ElevenLabs (via Vapi) voice used by deployed assistants when the agent's
  // voice selection doesn't resolve to a known voice id. Omit to let Vapi use
  // its account-default voice (deploy + calls still work).
  VAPI_DEFAULT_VOICE_ID: z.string().optional(),
  // Voice provider sent to Vapi alongside VAPI_DEFAULT_VOICE_ID (ElevenLabs by
  // default). Architect-chosen voiceProvider on the node overrides this.
